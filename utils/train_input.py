...
## OO4AL-input
def cross_fusion_data_input(x, y, index=None, alpha=0.8, use_cuda=True, lam1=None, lam2=None):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if lam1 == None:
        if alpha > 0:
    ...
    mixed_x1 = lam1 * x + (1 - lam1) * x[index, :]
    mixed_x2 = (1 - lam1) * x + lam1 * x[index, :]

    mixed_x = lam2 * mixed_x1 + (1 - lam2) * mixed_x2
    y_a, y_b = y, y[index]

    return mixed_x, y_a, y_b, lam1, lam2

def train_epoch_for_OO4AL_input(models, criterion, optimizers, dataloaders, alpha=0.8, layer=1):
    models['backbone'].train() 
    ...
    for data in dataloaders['train']:
        ...
        labels = data[1].cuda()
        # make the pair
        inputs, targets_a, targets_b, lam1, lam2 = cross_fusion_data_input(inputs, labels, alpha=alpha, use_cuda=True)
...
def train(models, criterion, optimizers, schedulers, dataloaders, args):
    ...
    for epoch in tqdm(range(args.epoch_num)):
        ...
        if args.approach in ['OO4AL', 'CMaM']:
            train_epoch_for_CMaM(models, criterion, optimizers, dataloaders, alpha=0.4, layer=args.layer)
        elif args.approach in ['OO4AL_SELF']:
            train_epoch_for_LL4AL_FixMatch(models, criterion, optimizers, dataloaders, alpha=0.4, layer=args.layer)
        elif args.approach in ['OO4AL_Input']:
            train_epoch_for_OO4AL_input(models, criterion, optimizers, dataloaders, alpha=0.4, layer=args.layer)
            ...
            train_epoch_for_Mixup(models, criterion, optimizers, dataloaders, alpha=0.4)
